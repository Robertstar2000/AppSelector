...
function App() {
      ...
      try {
        ...
        try {
          const [autoBackupResponse, lastBackupResponse] = await Promise.all([
            axios.get(`${apiUrl}/api/settings/auto_backup`),
            axios.get(`${apiUrl}/api/settings/last_backup_date`)
          ]);
          ...
          if (autoBackupEnabled && appsData.length > 0 && !lastBackup) {
            try {
              ...
              await axios.post(`${apiUrl}/api/auto-backup`, backupData);
  ...
  const saveApps = async (newApps: AppDefinition[]) => {
    ...
    try {
      await axios.put(`${apiUrl}/api/apps`, appsWithPositions);
      setApps(appsWithPositions);

      // Auto-backup if enabled - send to server for file system storage
      const autoBackupResponse = await axios.get(`${apiUrl}/api/settings/auto_backup`);
      const autoBackupEnabled = autoBackupResponse.data.value === 'true';
  ...
  const updateLastBackupDate = async () => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:3001';
      const timestamp = new Date().toISOString();
      await axios.put(`${apiUrl}/api/settings/last_backup_date`, { value: timestamp });
  ...
  const handleSaveApp = async (app: AppDefinition) => {
    ...
    try {
      if (editingApp) {
        const response = await axios.put(`${apiUrl}/api/apps/${app.id}`, app);
        console.log('PUT response:', response);
      } else {
        console.log('Creating new app via POST:', app);
        const response = await axios.post(`${apiUrl}/api/apps`, app);
  ...
  const handleDeleteApp = async (id: string) => {
    const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:3001';
    try {
      await axios.delete(`${apiUrl}/api/apps/${id}`);
      // Refetch apps
      const response = await axios.get(`${apiUrl}/api/apps`);
      setApps(response.data);
